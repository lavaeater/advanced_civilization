    ...
    if let Ok((trade_offer_entity, trade_offer)) = published_offer_query.get(trigger.event().entity) {
        if let Ok(ui_list) = published_offers_list.single() {
            ...
            ui_builder
                ...
                // Add initiator name
                .child()
                .with_default_text(format!("Trade offer from: {}", trade_offer.initiator_name))
                .parent()
                ...
                .child()
                .with_default_text("Offers:")
                .parent()
                .with_children(|builder| {
                    for (commodity, count) in trade_offer.initiator_pays.iter() {
                        builder
                            .child()
                            .with_default_text(format!("{}: {}", commodity, count))
                            .parent();
                    }
                })
                .with_children(|builder| {
                    for (commodity, count) in trade_offer.initiator_pays_guaranteed.iter() {
                        builder
                            .child()
                            .with_default_text(format!("{}: {} (Guaranteed)", commodity, count))
                            .parent();
                    }
                })
                .parent()
